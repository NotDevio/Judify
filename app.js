...
const SUPABASE_URL = 'https://vtcingxjtirзxcpkfcmr.supabase.co'.replace('з','z');
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ0Y2luZ3hqdGlyenhjcGtmY21yIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ3MDI0ODQsImV4cCI6MjA5MDI3ODQ4NH0.dsJJxwvY631112MaE5HeOLUQvfUI75fbDeOSe5wo_L0';
...
function playlistToRow(pl) {
  return {
    ...
    name: pl.name,
    cover: pl.cover || null,
    song_ids: JSON.stringify(pl.songs || []),
...
function openPlaylist(id) {
  ...
  const coverEl = document.getElementById('playlistCover');
  const firstSong = pl.songs.length > 0 ? songs[pl.songs[0]] : null;
  if (pl.cover) coverEl.innerHTML = `<img src="${pl.cover}">`;
  ...
  const totalDur = pl.songs.reduce((sum, idx) => sum + (songs[idx]?.duration || 0), 0);
  document.getElementById('playlistMetaText').textContent = `${pl.songs.length} songs, ${formatDuration(totalDur)}`;
  renderTrackList(pl);
  renderSidebarIcons();
  document.getElementById('npBottomPlaylistName').textContent = pl.name;
}
...
function setSort(type, el) {
  ...
  if (currentPlaylistId && currentPlaylistId !== '__added__') { const pl = playlists.find(p => p.id === currentPlaylistId); if (pl) renderTrackList(pl); }
...
async function ctxRemoveFromPlaylist() {
  ...
  if (pos !== -1) pl.songs.splice(pos, 1);
  await dbPutPlaylist(pl);
  ...
  playlists.filter(pl => pl.id !== exclude).forEach(pl => {
    ...
    item.onclick = async () => {
      if (action === 'add_song' && ctxTarget?.type === 'song') {
        if (!pl.songs.includes(ctxTarget.songIndex)) pl.songs.push(ctxTarget.songIndex);
      } else if (action === 'merge_playlist') {
        const src = playlists.find(p => p.id === ctxTarget.playlistId);
        if (src) src.songs.forEach(idx => { if (!pl.songs.includes(idx)) pl.songs.push(idx); });
      }
      await dbPutPlaylist(pl);
...
async function submitCreatePlaylist() {
  ...
  const pl = { id: 'pl_' + Date.now() + '_' + Math.random().toString(36).slice(2), name, songs: [], cover: null, userId: currentUser.id, isPublic: false };
  playlists.push(pl);
  await dbPutPlaylist(pl);
...
function onPlaylistSearch() {
  ...
  if (!q) {
    ...
    if (currentPlaylistId === '__added__') {
      ...
      const pl = playlists.find(p => p.id === currentPlaylistId);
      if (pl) renderTrackList(pl);
    }
    return;
  }
  // Filter tracks by search query
  let songIndices = currentPlaylistId === '__added__' ? songs.map((_, i) => i) : (playlists.find(p => p.id === currentPlaylistId)?.songs || []);
